...
app6 = FastAPI(title='Seminar_6, Task 1')
...
async def create_user(user: UserInSchema) -> dict:
    ...
    user_dict = user.model_dump()  # преобразует данные пользователя из схемы UserInSchema в словарь
...
async def create_product(product: ProductSchema) -> dict:
    ...
    product_dict = product.model_dump()  # преобразует объект product в словарь
...
async def update_user(user_id: int, user: UserInSchema) -> UserSchema:
    ...
    db_user = await db.fetch_one(query)
    if db_user:
        updated_user = user.model_dump(exclude_unset=True)  # указывает на то, что в результирующем словаре updated_user должны быть исключены поля, значения которых не были установлены в исходном объекте user
    ...
    async def virgin_db():
        ...
        for i in range(10):
            new_order = {
                ...
                "status": random.choice(["новый", "в ожидании", "готов к выдаче"])
            }
